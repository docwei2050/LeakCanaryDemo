...
public final class AnalyzedHeap {
  ...
  public static File save(@NonNull HeapDump heapDump, @NonNull AnalysisResult result) {
    ...
    try {
      ...
      Log.d("leak1", "Could not save leak analysis result to disk.");
    } finally {
      if (fos != null) {
        try {
          fos.close();
  ...
  @Nullable public static AnalyzedHeap load(@NonNull File resultFile) {
    ...
    try {
      ...
      if (deleted) {
       Log.d("leak1", "Could not read result file %s, deleted it."+resultFile);
      } else {
          Log.d("leak1", "Could not read result file %s, could not delete it either."+resultFile);
      }
    } finally {
      if (fis != null) {
        try {
